...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            message.reply_text(
                "Yeah! Like you can ban my Maestro. LMAO. He is kind, don't be hard on him?"
            )
        elif user_id in DEV_USERS:
            message.reply_text("That's one of the conductor, I can't ban him/her..")
        elif user_id in DRAGONS:
            message.reply_text("Musicarts can't be banned. Make a Note for it.")
        elif user_id in DEMONS:
            message.reply_text(
                "Hmmm! If I will ban a D2 Slayer, then who will protect the people.."
            )
        elif user_id in TIGERS:
            message.reply_text("You know that's a defender, you can't ban him/her..")
