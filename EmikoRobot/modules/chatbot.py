...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            ...
            return
        await lel.edit(
            f"Unmei AI Actived by {message.from_user.mention()} for users in {message.chat.title}"
        )
        ...
        if not Escobar:
            ...
            return
        await lel.edit(
            f"Unmei AI Deactivated by {message.from_user.mention()} for users in {message.chat.title}"
        )
...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        ...
        test = test.replace("Unmei", "Aco")
        test = test.replace(
            "My god is @yameteee_yamete_kudasai",
            "I'm a Musicart named after Beethoven's Symphony No. 5 in C Minor Op. 67",
        )
        ...
        test = test.replace(
            "I live in @unmei_support.", "I live in San Francisco, California."
        )
        test = test.replace(
            "I was created by @yameteee_yamete_kudasai", "I made myself"
        )
        test = test.replace(
        ...
        test = test.replace(
            "@yameteee_yamete_kudasai is my owner", "Have the control right."
        )
        test = test.replace(
        ...
        response = response.replace(
            "I'm a Christian", "My god is @yameteee_yamete_kudasai"
        )
        response = response.replace("9", "16")
        response = response.replace(
            "I'm married to my job.", "I'm married with @yameteee_yamete_kudasai"
        )
        response = response.replace(
            "I'm single.", "My husband is @yameteee_yamete_kudasai"
        )
        response = response.replace("Emikobot.ai", "@unmei_support")
        response = response.replace(
            "I live in San Francisco, California.", "I live in @unmei_support."
        )
        response = response.replace(
            "I made myself", "I was Created by @yameteee_yamete_kudasai"
        )
        response = response.replace(
            "Hi, my friend! Do you want me to tell you a joke?",
            "Hello there I am Unmei...nice to meet u",
        ...
        response = response.replace(
            "Have the control right.", "@yameteee_yamete_kudasai is my owner."
        )
        response = response.replace(
            "Hi, my friend, what can I do for you today?",
            "Hi, My name is Unmei, Nice to meet you",
...
@pbot.on_message(
    filters.regex("Unmei|unmei|robot|UNMEI|Destiny|destiny|DESTINY|Shoto|shoto")
    & ~filters.bot
    & ~filters.via_bot
    & ~filters.forwarded
    & ~filters.reply
